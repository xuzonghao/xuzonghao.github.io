...
运行结果：使用curl -I显示header
# curl -I http://10.210.128.192/php/error.php
...
**跳转网站**
<?php
header("Location: http://xuzonghao.github.io/"); /* Redirect browser */   
...
其中void表示没有参数
在调用seeion_start()函数前，程序不能有任何输出，否则php将出现警告信息
```

```
**启动session并将相关变量存储在session中**
<?php
session_start();        //打开session
$_SESSION['username']="username";     //将字符串存储在session中
?>
```
...
$var为要进行检测的变量，存在返回true，否则返回false。
```

```
**isset检测是否存在指定的session**

<?php
session_start();
$_SESSION['username'] = "username";
if(isset($_SESSION["username"])){
  echo "username已存在于session中，其值为：" . $_SESSION['username'] . ".";
}else{
  echo "username不存在。";
}
?>
...
参数var为要注销的session名称，函数释放给定的变量，即将指定的session注销
```

```
**使用unset函数注销已有的session**
<?php
session_start();        //启动session功能
$_SESSION['username'] = "username";     //注册一个session
unset($_SESSION['username']);       //注销已经注册的session
if(unset($_SESSION['username'])){     //检测是否已注销指定的session
  echo "注销失败.";
}else{
  echo "注销成功.";
}
...
**注意：不能使用unset($_SESSION)函数注销指定的SESSION,会将超全局变量$_SESSION销毁**
**session_register()、session_unregister()、session_is_registered()在php5下不在默认使用，处于安全考虑register_globle设置为off，**
```

```
**注销所有session**
bool session_destroy(void)
改函数将结束当前会话，并情况会话中所有资源，并总是返回true

**使用session_destroy注销所有session用例**
<?php
session_start();
$_SESSION['username'] = "username";
session_destroy();
...
<?php
session_start();
$_SESSION['username'] = "username";     //赋值全局数组
print_r($_SESSION); 
?>
```
...
<body>
  <form method="post" action="13.15.php" name="form1">
    <table>
      <tr><td>用户名:<input type="text" name="username"></td></tr>
      <tr><td>密码:<input type="password" name="password"></td></tr>
    </table>
...
if(($username=='username') && ($password == "password")){
  ...
  echo "用户名或密码错误，请重新<a href='13.14.php'>登陆</a>.";
}
?>
```
...
session_start();
if(isset($_SESSION['logined']) && $_SESSION['logined']){
  echo $_SESSION['username'] . "您好，欢迎光临！<a href='13.17.php'>登出</a>";
}else{
  echo "未登录！请<a href='13.14.php'>登陆</a>.";
}
?>
```
...
session_start();
session_destroy();
echo "注销成功！<a href='13.14.php'>登陆</a>.";
?>
```
...
<body>
  <form id="form1" name="form1" method="post" action="13.19.php?action=login">
    <p>&nbsp;</p>
    <p>&nbsp;</p>
    <p>&nbsp;</p>
    <p>&nbsp;</p>
...
if($c_action=="login"){
  if($c_pwd==" "){
    echo "<script type='text/javascript'> alert('密码不能为空！')；location.href="/13.19.php";</script>";
    exit;
  }
  if($c_pwd != "phpcoding.cn"){
    echo "<script type='text/javascript'> alert('密码错误！');location.href="13.19.php";</script>";
    exit;
  }
